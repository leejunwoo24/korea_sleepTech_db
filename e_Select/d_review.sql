...
# 1) 이름(name)으로 알파벳 순 정렬and

select * from `members` order by name;  -- 오름차순 asc는 생략 가능 (desc는 반드시 명시!)
...
select gender, avg(points) avg_points -- as 키워드 생략 가능 / 집계 함수: avg(), sum(), max(), min() count()
from `members`
group by gender;
