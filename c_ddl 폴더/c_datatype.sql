...
/*
    ...
    c) int (4byte: 32bit)
    : 약 -21억 ~ 약 21억
    : 일반적인 용도로 가장 많이 사용되는 정수형 타입alter
    EX) 주문 번호, 고객 ID 등
...
insert into `integer`
value (127, 32767, 200000000, 90000000000);

select * from `integer`;

# Out of range 오류: 해당 데이터 타입의 범위를 벗어나는 경우 발생
-- insert into `integer`
-- value (128, 32767, 2000000, 90000000000);alter
...
/*
    ...
    cf) char: character 문자
		varchar: varialbe character field 가변 길이 문자열
...
/*
    ...
    a) float
		: 소수점 아래 2자리까지 표현 (정수 5자리)
        EX) 시력, 가격 등 
        
    b) double
		: 소수점 아래 4자리까지 표현 (정수 6자리) 
        - 구체적인 값을 표현할 수 있음
