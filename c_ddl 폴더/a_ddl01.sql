...
### DDL 문법 정리 ###
# create, alter, drom, truncate

-- 데이터베이스 생성 --
### create: 데이터베이스 생성, 데이터를 저장하고 관리하는 첫 단계
# 기본 형태
# create dateabase 데이터베이스명; 
...
create database school;
 ...
 -- cf) 문자 인코딩을 추가한 테이블 생성
 # : UTF-8 문자 인코딩을 사용하여 한글 등의 문자 정보를 올바르게 저장할 수 있도록 설정
 create table `school`.`students_encoidng`(
		student_id int,			 -- 학생 고유번호 (정수형)
        student_name char(8),    -- 학생 이름 문자 (최대 8자리)
        sutdent_gender char(8)  -- 학생 성별 (최대 8자리)
 )
 default character set = utf8;

# cf) 데이터베이스명은 중복될 수 X, 같은 이름의 DB, Table 생성 불가!
