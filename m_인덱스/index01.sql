...
# EX) 책에서 원하는 단어를 차을 때 사용하는 `찾아보기 (색인)`

-- 1) 인덱스 종류 -- 
# 1) 클러스터형 인덱스
# : 기본키(PK) 설정 시 자동 생성
# - 테이블 당 (복합키는 2개 이상의 컬럼 덩어리를 하나의 primary key로 보는 것)
# - 정렬 기능을 포함: 지정한 열(기본키 설정 열)을 기준으로 자동 정렬
...
select * from members; -- 클러스터형 인덱스가 없는 경유(PK값이 없는 경우): 데이터 삽입 순서대로 출력
...
-- 정렬 되지 않는 보조 인덱스 --
# : 고유 키 지정 시 생성 / 테이블 당 여러 개 기능 정렬 X
