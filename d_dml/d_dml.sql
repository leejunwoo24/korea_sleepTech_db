...
select * from `example02`; # 컬럼의 순서, 개수, 자료형이 반드시 일치해야 
...
#  MySQL에서 안전 모드(SAFE UPDATES)를 비활성화하는 설정
/*
MySQL에서 SQL_SAFE_UPDATES 모드는 초보자나 실수 방지를 위해 다음과 같은 제한을 두는 안전 장치입니다:

UPDATE나 DELETE 문을 실행할 때 WHERE 절이 없거나,

기본 키 또는 인덱스를 사용하지 않는 조건이 있는 경우,
