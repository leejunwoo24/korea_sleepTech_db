...
-- insert into `students` # 기본키는 중복될 수 없다
-- values
-- 	(1, '조승범', 'C전공');
...
insert into `orders`
values
	(1, '2025-04-11', 123); # `orders` 테이블의 FK는 members의 PK인 123 데이터를 참조하고 하고 있다. 이런 경우 FK의 제약조건은 제거할 수 없다
...
#   - `orders`테이블에 DDL명령어 truncate를 통해 테이블의 모든 데이터를
#  비워줌으로서 `orders`테이블의 FK가 참조해온 데이터를 모두 비워줘야 한다.
truncate table `orders`;

# - 제약 조건 이름 검색이 필수
# 외래 키 제약 조건 이름 확인 #
select constraint_name  # 제약 조건의 이름을 조회
from information_schema.key_column_usage # 스키마의 메타데이터 중 열에 대한 제약 조건을 가져온다.
# where 조건 내에서 테이블명과 컬럼명을 데이터처럼 사용: 따옴표 지정
where table_name = 'orders' and column_name = 'member_id'; # 테이블의 이름이 'orders' 이고 컬럼의 이름이 'member_id'인 것으르 필터링

# >>> 'orders_ibfk_1'

# 1) '기본 테이블의' 외래 키 삭제
alter table `orders`
drop foreign key orders_ibfk_1; # 외래키의 경우 여러개가 지정될 수 있기 때문에 기본키와 다르게
								# foreign key만 입력하는 것이 아닌 구체적인 외래키 제약조건의 이름까지 입력해준다.
