...
create table if not exists `student_course` (
    ...
    #  >> 해당 각각의 컬럼이 컬럼 내에서는 중복 가능하지만, 둘을 합친 값으 ㄴ테이블 내에서 고유
...
-- insert into `student_course` (course_id, course_null, professor_name)
-- 	values
--     (105, '코리아IT', '조승범');
...
create table student_course_connect(
	student_id int,
    course_id int,
    primary key (student_id, course_id), -- 복합키를 설정
...
values 
	...
	(1, 102),
	(2, 101),
	(3, 103);  # 복하키 설정으로 두 개의 컬럼을 합친 내용이 다른 레코드와 중복 X
