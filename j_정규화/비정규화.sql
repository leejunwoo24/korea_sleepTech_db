...
# - 정규화를 뒤집는 개념 X, 특정 상황의 성능을 위해 설계적으로 일부 정규화를 되돌리는 작업 

# cf) 정규화: 테이블의 중복을 없에는 것
...
create table customers (
    ...
    name varchar(100), -- 컬럼명으로 name 보다는 '테이블_name' 사용을 권장
    address varchar(255)
);
...
# 2) 단점: 데이터를 조회할 때마다 여러 테이블을 조인해야 하므로 성능 저하 기능
