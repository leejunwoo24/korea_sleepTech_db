/*
    ...
    1) 대소문자 구분을 하지 않지만 / '소문자 사용을 권장'
		- Linux: 대소문자를 구분
        - Window, MacOS: 대소문자 구분 안 함
        
	2) 구분자: _언더스코어 사용(snake case 사용)
    
    DB명 : class_enrollment
    테이블명: students, classes 등
    컬럼명: students, created_at 등
    
    3) 약어 사용: 가능한 사용 지양
		- 가독성을 위해 줄이지 않고 전체 단어 사용 권장
        
        EX) addr(address), amt(amount), qty(quantitu), tel_no(phone_number) 등
...
/*
    ...
    1) 복수형 명사 권장 : users, products, orders 등
						user_reviews, order_items 등alter
...
# DB명: bookhub_db, senior_db, body_laboratory_db
