...
# : 정규화의 세 번째 단계
# : 2NF(제 2정규형)을 만족하는 테이블에서, 모든 비기본 속성이 기본키에만 함수적으로 종속
# - 이행적 종속성을 제거

# cf) 이행적 종속성
# : 어떤 속성 A가 다른 속성 B에 종속되고
#	, B가 또 다른 속성 C에 종속된 경우
# >> A가 C에 이행적으로 종속됨alter

# 제 3 정규형은 '이행적 종속성'을 제거
...
/*
    ...
    - 기본키(PK): 학번
    - 비기본 속성: 이름, 학과 ID, 학과명, 학과위치
    
    학번 > 학과 ID (학생이 소속한 확과)
    학과ID > 학과명, 학과위치 (학과 ID로 학과정보 확인 가능)

	: 학번 > 학과명, 학과위치 - 직접적이지 않음
    : 학과 > 학과ID > 학과명, 학과위치 (이행적 종속성이 존재) 
...
create table students_3 (
    ...
    name varchar(100),
    department_id int, -- 학과 ID만 저장 (학생 정보가 생략)
